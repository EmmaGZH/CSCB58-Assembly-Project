...
#
# Student: Emma Gong, 1007819951, gongemma, emma.gong@mail.utoronto.ca
#
...
level_three:	li $t0, BASE_ADDRESS
		li $t1, BLACK
		li $t2, 4
		addi $t3, $t0, 16384
		jal color
...
start_three:	li $t0, BASE_ADDRESS
		li $t1, BLACK
		li $t2, 4
		addi $t3, $t0, 16384
		jal color
...
# ENEMY MOVING
#########################################
game_move:	beq $s1, 1, game_1_move
...
bullet_right: 	beq $t8, 0, new_bullet
		beq $t8, $t4, bullet_disappear
		sw $zero, 0($t8)
		addi $t8, $t8, 4
		sw $t0, 0($t8)
		sw $t8, 0($t2)
...
bullet_left:	beq $t8, 0, new_bullet
		beq $t8, $t4, bullet_disappear
		sw $zero, 0($t8)
		addi $t8, $t8, -4
		sw $t0, 0($t8)
...
fly_right:	lw $t8, 4($t2)
		...
		lw $t8, 4($t2)
		lw $t8, 284($t8)
		beq $t8, LIGHT_BLUE, fly_over_right
		lw $t8, 4($t2)
       		sw $zero, 0($t8)
        	addi $t8, $t8, 4
        	sw $t0, 0($t8)
...
fly_left:	lw $t8, 4($t2)
		...
		lw $t8, 4($t2)
		lw $t8, 256($t8)
		beq $t8, DARK_BLUE, fly_over_left
...
##takes in value $t2, $t3, $t4        		
stoneman_move_right:	lw $t8, 4($t2)
			beq $t4, $t8, stoneman_move_left
			li $t8, 1
			sw $t8, 0($t2)
			lw $t8, 4($t2)
			lw $t8, 16($t8)
			beq $t8, LIGHT_BLUE, sm_jump_over_right
...
stoneman_move_left:	lw $t8, 4($t2)
			beq $t3, $t8, stoneman_move_right
			li $t8, 2
			sw $t8, 0($t2)
			lw $t8, 4($t2)
			lw $t8, -16($t8)
			beq $t8, DARK_BLUE, sm_jump_over_left
...
respond_d: 	#Respond to key press 'a' event   -4
        	#check whether the ship has reach the left end
        	lw $t0, 20($a0)  #1st unit
        	li $t1, 256
        	div $t0, $t1
        	mfhi $t1
       		beq $t1, 252, return
...
respond_w: 	#Respond to key press 'a' event   -4
        	#check whether the ship has reach the left end
        	lw $t0, 0($a0)  #1st unit
        	addi $t0, $t0, -256
...
respond_p:	j level_one
        	
falling: 	#Respond to key press 'w' event   -256
        	#check whether the ship has reach the left end
        	lw $t0, 40($a0)  
        	addi $t0, $t0, 256
        	bgt $t0, 0x1000C000, fall_from_top
        	lw $t0, 0($t0)
        	li $t1, WHITE
        	beq $t0, $t1, return
        	lw $t0, 44($a0)  
...
next_level:	beq $s1, 1, level_two
		beq $s1, 2, level_three
		beq $s1, 3, win

Ini_Wizard:	la $t4, wizard
		li $t0, PURPLE
		li $t1, SKIN
		addi $t5, $gp, 13952
		sw $t5, 0($t4) #unit 1
...
game_1_coin: 	li $t0, BASE_ADDRESS
		li $t1, GOLDEN_YELLOW
		li $t2, YELLOW
...
game_1_diamond:	li $t0, BASE_ADDRESS
		li $t1, LIGHT_BLUE
		li $t2, DARK_BLUE
		li $t3, BLUE
		sw $t1, 1144($t0)
...
game_1_enemy:	li $t0, BASE_ADDRESS
		li $t1, GRAY
		li $t2, DARK_GRAY
		la $t4, game1_enemy1
		sw $t1, 1216($t0)
...
game_2_coin:	li $t0, BASE_ADDRESS
		li $t1, GOLDEN_YELLOW
		li $t2, YELLOW
...
game_2_diamond:	li $t0, BASE_ADDRESS
		li $t1, LIGHT_BLUE
		li $t2, DARK_BLUE
		li $t3, BLUE
		sw $t1, 1092($t0)
...
game_2_enemy:	li $t0, BASE_ADDRESS
		li $t1, GREEN
		li $t2, ORANGE
		la $t4, game2_enemy1
		sw $t1, 768($t0)
...
game_3_diamond:	li $t0, BASE_ADDRESS
		li $t1, LIGHT_BLUE
		li $t2, DARK_BLUE
		li $t3, BLUE
		sw $t1, 780($t0)
...
game_3_coin:	li $t0, BASE_ADDRESS
		li $t1, GOLDEN_YELLOW
		li $t2, YELLOW
...
game_3_moving_enemy:	li $t1, GREEN
			li $t2, ORANGE
			sw $t1, 1328($gp)
			sw $t1, 1332($gp)
			sw $t1, 1352($gp)
